...
class Create_Course53 extends StatefulWidget {

  @override
  State<Create_Course53> createState() => _Create_Course53State();
  final String courseId;
  final String doctor;

  Create_Course53({required this.courseId,required this.doctor});
}

class _Create_Course53State extends State<Create_Course53> {
  @override
                              ...
                              builder: (context) => defaultButton(
                                function: () {
                                  if (formKey.currentState!.validate()) {
                                    // Update the previous professor's document to remove the course

                                    FirebaseFirestore.instance.collection('users').doc(widget.doctor).update({
                                      'course': FieldValue.arrayRemove([widget.courseId]),
                                    });
                                    ...
                                    FirebaseFirestore.instance.collection('courses').doc(widget.courseId).update({
                                      ...
                                      'doctor_id': ProffesereController.text,
                                    });
                                    ...
                                    FirebaseFirestore.instance.collection('users').doc(ProffesereController.text).update({
                                      'course': FieldValue.arrayUnion([widget.courseId]),
                                    });
